  ...
  while read NAMESPACE; do
    if [[ ! $NAMESPACE == *"NAMEHEADER"* ]]; then
      if [ ! -z $NAMESPACE ]; then
        ...
        kubectl --insecure-skip-tls-verify get modules -n $NAMESPACE -o json 2>/dev/null >$CAPTURE_DIR/cluster-snapshot/$NAMESPACE/modules.json || true
      fi
    fi
...
function full_k8s_cluster_snapshot() {
  ...
  if [ $? -eq 0 ]; then
    ...
    nsExists=$(kubectl get ns verrazzano-monitoring --ignore-not-found)
    if [ ! -z "$nsExists" ]; then
