...
func EnsureResourceLogger(config *ResourceConfig) (VerrazzanoLogger, error) {
	...
	zaplog, err := BuildZapInfoLogger(2)
	if err != nil {
...
func ForZapLogger(config *ResourceConfig, zaplog *zap.SugaredLogger) VerrazzanoLogger {
	...
	zaplog = zaplog.With(FieldResourceNamespace, config.Namespace, FieldResourceName,
		config.Name, FieldController, config.ControllerName)
