...
func AppendCondition(module *modulesplatform.ModuleAction, message string, conditionType modulesplatform.LifecycleCondition) {
	conditions := module.Status.Conditions
	newCondition := NewCondition(message, conditionType)
	var lastCondition modulesplatform.ModuleActionCondition
	if len(conditions) > 0 {
...
func NewCondition(message string, conditionType modulesplatform.LifecycleCondition) modulesplatform.ModuleActionCondition {
	t := time.Now().UTC()
	return modulesplatform.ModuleActionCondition{
		Type:    conditionType,
		Message: message,
...
func needsConditionUpdate(last, new modulesplatform.ModuleActionCondition) bool {
