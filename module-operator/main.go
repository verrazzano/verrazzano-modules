...
func main() {
	...
	if err := moduleaction.InitController(mgr, helmfactory.NewModuleActionHandlerInfo(), moduleapi.HelmModuleClass); err != nil {
		log.Errorf("Failed to start Helm controller", err)
		return
	}

	// init Calico lifecycle controller
	if err := moduleaction.InitController(mgr, calicofactory.NewModuleActionHandlerInfo(), moduleapi.CalicoModuleClass); err != nil {
		log.Errorf("Failed to start the Calico controller", err)
		return
	}

	// init CCM lifecycle controller
	if err := moduleaction.InitController(mgr, ccmfactory.NewModuleActionHandlerInfo(), moduleapi.CCMModuleClass); err != nil {
